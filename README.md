...
- The model picker's footer ("Mistral rates (EUR) as of ...") stays worded for Mistral even now that the agent form's picker (MYO-421) can list models of every provider with a configured key side by side, each shown in its own native currency (EUR for Mistral, USD for the others). Rewording it correctly is an i18n string change across the module's 4 locales, left out of MYO-421's scope to avoid touching translation catalogs incidentally; the model list itself and each option's price/currency are correct, only that one footer line can read oddly once a non-Mistral model is selected.
