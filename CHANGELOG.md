...
Le module passe de **0.4.4** à **0.4.5**. Couche données/API du centre de notifications agents en topbar BO (cadré CEO en MYO-481, implémentation MYO-483) : cette livraison couvre le sous-lot qui ne dépend pas de la maquette UX (état de lecture persisté, agrégation, endpoints JSON) — le hook topbar et le template restent au ticket sœur FrontendEngineer.

### État de lecture persisté + service d'agrégation + endpoints JSON — MYO-484

- Nouvelle table `agent_notification_ack` (`Config/update/0.4.5.sql`, `CREATE TABLE IF NOT EXISTS`) : `admin_id` (sans FK vers `admin`, même convention que `agent_staged_change.approved_by`), `source_type` (`staged_change` | `outbound_message`), `source_id`, `acked_at`, index unique `(admin_id, source_type, source_id)` — l'acquittement est un upsert idempotent, jamais de duplication ni d'erreur en rejouant le même ack.
- `Service/Notification/NotificationCenterService` : agrège les trois sources sans inventer de 4e table — `agent_staged_change` (statut `pending`), sa classification « Brief maintenant » via le `BriefUrgencyClassifier` déjà existant (MYO-472, réutilisé tel quel, y compris pour les avis notés en relisant `payload_before.rating` déjà enregistré à la mise en attente), et `agent_outbound_message`. Un item est « non lu » quand aucune ligne `agent_notification_ack` ne le couvre pour l'admin courant ; acquitter un item classé « Brief » acquitte la même ligne `agent_staged_change`, jamais un ack séparé par groupe visuel. Deux admins ne s'éteignent jamais mutuellement la pastille (isolation par `admin_id`).
- `Controller/Admin/NotificationCenterController` : `GET /admin/merchant-agent/notifications` (route `commerceagents_notifications`, compte total/par type + items non lus avec de quoi construire une destination cliquable) et `POST /admin/merchant-agent/notifications/ack` (route `commerceagents_notifications_ack`, body `{sourceType, sourceId}`). Même garde d'accès que le reste du module (`isGranted ADMIN + commerceagents VIEW`) et même mécanisme CSRF par en-tête que `MerchantChatController` (MYO-284 M4) — le jeton est exposé dans la réponse JSON de `summary()` faute de template propre à ce contrôleur pour porter un champ caché.
- 13 tests PHPUnit (service + contrôleur, y compris rejet anonyme 302 et rejet CSRF 403) exécutés réellement contre la base `test`.
- Corrige au passage un blocage `bin/test-prepare`/`composer test:prepare` découvert en travaillant sur ce ticket : `Config/update/0.4.4.sql` (MYO-475) exécutait `SELECT @var := ...` comme instruction autonome, ce qui casse `DatabaseSetup::applyModuleSchema()` (splitteur naïf + `PDO::exec()`) dès la 1ʳᵉ requête suivante (« Cannot execute queries while other unbuffered queries are active »), et empêchait `TheliaBlocks` (traité après `CommerceAgents` dans le parcours du dossier) d'être enregistré dans la base `test`. Remplacé par l'idiome `SET @var := (SELECT ...)`, sémantique identique, compatible `PDO::exec()`. N'affecte pas la voie de mise à jour réelle du BO (`ModuleManagement::updateModule()`), qui n'a jamais buté dessus.

## 0.4.6 (MYO-479/480/493)

Le module passe de **0.4.5** à **0.4.6**. Portée de ce bump : uniquement la livraison ci-dessous (repli du dock en bulle sur `/panier`). Il ne rattrape **pas** rétroactivement les commits fonctionnels déjà mergés sur `myorg` sans version bump depuis le tag `v0.4.3` (dont MYO-476/515 — carte opt-in newsletter, commits `9625dc2`/`7b4f1a7`) : cette dérive plus large reste suivie par le filet existant (`scripts/check-version-drift.sh`, MYO-452→465) et relève d'un arbitrage CTO séparé, hors périmètre de ce ticket.

### Repli du dock en bulle compacte sur la page panier — MYO-479 (spec), MYO-480/493 (implémentation)

- Sur `/panier` uniquement (route `checkout_cart`), le dock plein-largeur devient une bulle circulaire 56×56px ancrée bas-**gauche** (au lieu de bas-centre), à l'opposé du CTA « Commander » toujours aligné à droite de sa colonne. `.caw-collapsed`/`.caw-proactive` suivent le même ancrage à gauche, largeur plafonnée à 360px desktop (mobile : `calc(100vw - 1.5rem - 4rem)`, spec §3.2/§3.4). Toutes les autres pages : `.caw-dock` strictement inchangé (nouveau bloc `.caw-dock-cart` séparé, affiché uniquement quand `isCartPage` est vrai).
- Détection « page panier » : nouveau getter partagé `isCartPage` (`chat-widget.js`), qui reprend la comparaison déjà en place dans `trackCartOpen()` (`window.location.pathname` vs le chemin de `checkoutUrl`, déjà injecté par `ChatWidgetThemeHook`) au lieu de la dupliquer. Réévalué à chaque navigation Turbo Drive du tunnel (`turbo:load`, `checkout-base.html.twig` `data-turbo="true"`) via une propriété réactive `currentPath`, en plus de la réévaluation naturelle au premier rendu — flag d'affichage orthogonal à la machine à états `dock`/`collapsed`/`open` (spec §2), aucun nouvel état ajouté.
- Garde-fou non-régression CTA à deux niveaux (spec §5) : position bas-gauche **et** `padding-bottom` réservé sur la colonne récap (`.checkout-cart-page .bg-lightest`, `templates/frontOffice/flexy/assets/styles/layout.css`), scopé à `/panier` seul via une classe de corps posée par `checkout-cart.html.twig` (`checkout-base.html.twig` est partagé par tout le tunnel) — garantit par construction que le CTA, toujours en flux normal, ne peut jamais passer sous la bulle, sans mesure de collision au runtime.
- A11y (spec §6) : bouton natif `56×56px` (> 44px), `aria-haspopup="dialog"`, `aria-expanded` binaire, `aria-label` = clé existante `openAssistant` (aucune nouvelle chaîne pour ce point), focus visible repris tel quel de `.caw-quickreply:focus-visible`. `prefers-reduced-motion` : transitions ramenées à `opacity 0.01s linear` sans transform, pulse du point de notification supprimé.
- i18n (spec §7) : une seule clé réellement nouvelle, `New suggestion available` / `Nouvelle suggestion disponible` (déjà présente dans les 4 catalogues depuis `bc71106`, manquait seulement le câblage dans `ChatWidgetThemeHook::render()`), annoncée en `aria-live="polite"` (visuellement masquée) quand une suggestion proactive est disponible mais la carte hors champ — réutilise le point d'attention `caw-proactive-pulse` existant, déclenché une fois.
- Volet e-mail/opt-in (spec §8) : aucun nouveau point d'entrée — `.caw-proactive` reste le seul, hérite automatiquement de l'ancrage bas-gauche décrit ci-dessus.

### Migrations

Aucun fichier `Config/update/0.4.6.sql` : uniquement du CSS/JS/Twig/i18n, aucune surface de persistance touchée.
